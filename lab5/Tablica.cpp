...
int Tablica::sumuj_elementy_wiersza(int nr_wiersza) {
	...
	{
		wynik += this->t[nr_wiersza][i].get_wartosc() + this->t[nr_wiersza][i + 1].get_wartosc();
	}
	return wynik;
}
...
int Tablica::sumuj_elementy_kolumny(int nr_kolumny) {
	...
	{
		wynik += this->t[i][nr_kolumny].get_wartosc() + this->t[i + 1][nr_kolumny].get_wartosc();
	}
	return wynik;
}
...
int Tablica::najmniejsza_z_wiersza(int nr_wiersza) {
	int wynik = this->t[nr_wiersza][0].get_wartosc();
	for (int i = 1; i < this->liczba_kolumn; i++)
	{
		if (wynik > this->t[nr_wiersza][i].get_wartosc())
		{
			wynik = this->t[nr_wiersza][i].get_wartosc();
		}
	}
	return wynik;
}
...
int Tablica::najmniejsza_z_kolumny(int nr_kolumny) {
	int wynik = this->t[0][nr_kolumny].get_wartosc();
	for (int i = 1; i < this->liczba_wierszy - 1; i++)
	{
		if (wynik > this->t[i][nr_kolumny].get_wartosc())
		{
			wynik = this->t[i][nr_kolumny].get_wartosc();
		}
	}
	return wynik;
}
	...
	if (plik.is_open())
	{
		...
		while (getline(plik, wiersz))
		{
			...
			while (ss.good())
			{
				...
				nowa_tablica[x][y] = Komorka(stoi(element));
				y++;
			}
			x++;
		}
