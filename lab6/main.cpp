...
int main()
{
	...
	while (!zakonczyc_program)
	{
		...
		case 15:
			...
			cout << "Podaj wspolrzedna y elementu do wyswietlenia: " << endl;
			cin >> iy;
			if (ix < 0 || iy < 0 || ix >= tab.get_liczba_wierszy() || iy >= tab.get_liczba_kolumn())
			{
				cout << "W tablicy nie ma elementu o wspolrzednych (" << ix << "," << iy << ")!" << endl;
				break;
			}
			tab.wyswietl_element_liczbowy(ix, iy);
			break;
		case 16:
			int sx, sy;
			cout << "Podaj wspolrzedna x elementu do wyswietlenia: " << endl;
			...
			cout << "Podaj wspolrzedna y elementu do wyswietlenia: " << endl;
			cin >> sy;
			if (sx < 0 || sy < 0 || sx >= tab.get_liczba_wierszy() || sy >= tab.get_liczba_kolumn())
			{
				cout << "W tablicy nie ma elementu o wspolrzednych (" << sx << "," << sy << ")!" << endl;
				break;
			}
			tab.wyswietl_element_tekstowy(sx, sy);
			break;
		case 0:
		default:
